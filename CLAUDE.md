...
- **Cache versioning:** Bump `CACHE` in `pwa/sw.js` AND `CACHE_VERSION` in `pwa/js/api.js` together on every release. **Currently v77.** Also add any NEW js/css to the `SHELL` array in `sw.js`.
