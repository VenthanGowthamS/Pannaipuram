...
- **Bottom nav (`pwa/css/nav.css`) — v77:** exactly **5 tabs**, `display:flex` + `overflow:visible`, each `flex:1 1 0` (75px on a 375px phone). Order: 🚌 பேருந்து · 🛺 ஆட்டோ · **📰 சங்கமம்** · 🏥 மருத்துவம் · ➕ மேலும். **The count must stay ODD** — that is the only way சங்கமம் lands on the true centre; with six tabs the midpoint fell between two of them and always looked off. 📞 அவசரம் therefore lives in the ☰ drawer as `.menu-item[data-section="emergency"]` (styled `.menu-item-urgent`, red rail, first in the list); drawer items now support `data-section` (switch tab) as well as `data-sheet` (open sheet). `.nav-tab-news` is raised `margin-top:-16px` with a 50px circle — **that raise is only safe while the bar is `overflow:visible`**. If the bar is ever made a scroller again, `overflow-x:auto` forces `overflow-y` to a clipping value, the raised part gets cut off and the button becomes untappable (this happened in v75).
...
- **Cache versioning:** Bump `CACHE` in `pwa/sw.js` AND `CACHE_VERSION` in `pwa/js/api.js` together on every release. **Currently v80.** Also add any NEW js/css to the `SHELL` array in `sw.js`.
- **v80 (July 2026) — image alone is enough to post; no more crop.**
  - `title_tamil`/`content_tamil` are only mandatory when there is **no image** — a photo of a burst pipe or a torn road is content on its own. Rule lives once in `backend/src/routes/bulletin.js` `validatePostContent()`, exported and reused by `routes/admin/bulletin.js` so the official-post route can't drift from the villager route. Both still require *something* (image OR real text) — an empty post with neither is rejected.
  - PWA: the `*` next to தலைப்பு/விபரம் hides and a "(படம் இருந்தா வேண்டாம்)" hint appears the moment a photo is attached (`toggleTextRequirement()` in `pwa/js/bulletin.js`), on both new posts and edits.
  - **Fixed while building this:** editing a photo post used to silently delete the photo. The file `<input>` has nothing in it until the user re-picks a file, so submitting an edit without touching it sent `image_url: null` and wiped the image. `startEdit()` now preloads the existing photo into a module-scope `_pendingImage` (was a function-local var, invisible to `startEdit`), shows it in the preview with a "✕ படம் நீக்கு" button, and only clears it if that button is tapped or a new file is chosen.
  - **Fixed the actual crop complaint:** `.bl-img` and `.bl-img-preview img` used `object-fit: cover`, which crops whatever doesn't fit the box — so a shared photo could have its subject cut off. Both are now `object-fit: contain` (letterboxed on the grey background, never cropped). The admin panel's `<img>` tags were already crop-free (`maxWidth`/`maxHeight` only, no `object-fit`).
  - Tests: 6 new cases across `backend/test/bulletin.test.js` — image-only submit/edit succeed, no-image-no-text still rejected (both public and official-post routes).
- **v77 (July 2026) — villager self-service edit/delete + 5-tab nav.**
  - `PATCH /api/bulletin/:id` and `DELETE /api/bulletin/:id`. Ownership is proved by **poster_id + the phone given at registration** — there is no login, and poster_id alone is a small sequential integer, so it is guessable on its own. "Not yours" and "wrong phone" both return the same 403 so post ids can't be mapped to villagers. The official account is refused on both routes (admin panel only).
  - **An untrusted villager's edit sets the post back to `pending`.** Without this, someone gets a harmless post approved and then rewrites it into spam that is already live. Trusted posters stay `approved`. The response carries `requeued` so the PWA can explain the wait.
  - `GET /api/bulletin?poster_id=` additionally returns **that poster's own pending posts** (never anyone else's), so a post awaiting approval doesn't look deleted to its author. `pwa/js/bulletin.js` shows those with a ⏳ chip and an orange rail.
  - Deleting does **not** refund the daily allowance — otherwise post → delete → post loops around the 1-per-day limit. There is a test for exactly this.
  - `pwa/js/api.js`: `apiPost` is now a thin wrapper over `apiSend(method, …)` so PATCH/DELETE share the 20s timeout and Tamil error handling.
  - The stored poster object now includes `phone`. Villagers registered before v77 have no phone saved, so their edit/delete controls stay hidden until they re-register via "மாத்து" — deliberate, not a bug.
...
| **Community Bulletin (v71)** — 📰 சங்கமம் tab, user registration (phone+name), post news/images (optional), 7-day auto-purge, admin moderation tab | pwa/js/bulletin.js, pwa/css/bulletin.css, pwa/index.html, backend/src/routes/bulletin.js, admin Bulletin.jsx | ✅ v71 |
...
*Last updated: July 9, 2026 — PWA v69: acting-driver DL-verified badge, doctor-level More Details (one 📝 box instead of per-day duplicate notes). Pending Supabase migrations: driver_photos, acting_license, doctor_notes.*
